...
# V2.5L conservative HEV longitudinal smoothing.
#
...
#
# V2.5L keeps the known V2.5J CAN layout and disabled/ready frames, but stays
# inside the most common stock moving-brake region: pump -0.4/+0.4 and a
# maximum 0.75 m/s^2 deceleration command. It does not use the unverified
# 0x30/0x31 stop/hold states.
#
# Hybrid-specific safety rule: negative planner acceleration is allowed to
# request regen or hydraulic braking only while a persistent camera lead is
# present. With no lead, the desired-speed command follows current speed and
# cannot create cruise-overspeed or engagement-transient braking.
V25L_BRAKE_MIN = 0.05
V25L_BRAKE_ENTRY = 0.25
V25L_BRAKE_RELEASE = 0.08
V25L_BRAKE_URGENT_ENTRY = 0.60
V25L_BRAKE_MAX = 0.75
V25L_BRAKE_STEP_UP = 0.05
V25L_BRAKE_STEP_DOWN = 0.01
V25L_BRAKE_FILTER_UP = 0.25
V25L_BRAKE_FILTER_DOWN = 0.08
...
V25L_PROPULSION_DWELL_FRAMES = 50 # 0.50 seconds at the 100 Hz frame clock
...
V25L_REGEN_ENTRY = 0.10
V25L_REGEN_MAX_OFFSET = 0.10      # m/s below vEgo; intentionally very mild
V25L_REGEN_STEP = 0.01            # m/s per 20 Hz update

V25L_ACCEL_ENTRY = 0.10
V25L_ACCEL_ENTRY_FRAMES = 3
V25L_ACCEL_CAP_LEAD = 0.12
V25L_ACCEL_CAP_NO_LEAD = 0.20
V25L_ACCEL_STEP_UP = 0.01         # m/s^2 per 20 Hz update
V25L_ACCEL_STEP_DOWN = 0.03
V25L_SPEED_OFFSET_CAP_LEAD = 0.15
V25L_SPEED_OFFSET_CAP_NO_LEAD = 0.25
...
def v25l_rate_limit_accel(target, last):
  if target > last:
    return min(target, last + V25L_ACCEL_STEP_UP)
  return max(target, last - V25L_ACCEL_STEP_DOWN)
...
class CarController():
  def __init__(self, dbc_name, CP, VM):
    ...
    # V2.5L: simple lead-qualified brake/regen state and a separately smoothed
    # positive acceleration command.
    ...
    self.v25l_regen_offset = 0.0
    self.v25l_accel_counter = 0
    self.v25l_accel_cmd = 0.0
  ...
  def update(self, enabled, active, CS, frame, actuators, pcm_cancel_cmd,
    ...
    if (frame % 5) == 0:  # Send ACC/HUD messages every 5 frames
      ...
      # Qualify camera leads before allowing any negative longitudinal command.
      # Resetting this counter on each engagement prevents a stale/one-frame
      # model detection from causing the no-lead SET slowdown seen in V2.5K.
      if lead:
      ...
      # -----------------------------
      # V2.5L lead-qualified hydraulic braking
      # -----------------------------
      ...
      release_hydraulic = (
        not longitudinal_allowed or
        not lead or
        CS.out.vEgo <= V25L_MIN_MOVING_SPEED or
        brake_request < V25L_BRAKE_RELEASE
      )

      if self.v25l_brake_active and release_hydraulic:
        # Release immediately when the lead disappears or the planner stops
        # requesting meaningful deceleration. Do not repeat V2.5K's 0.75-second
        # brake hold through a positive acceleration request.
        self.v25l_apply_brake = 0.0
        self.v25l_brake_target = 0.0
        self.v25l_brake_active = False
        self.v25l_brake_entry_counter = 0
      ...
      if not self.v25l_brake_active:
        ...
        if (
          urgent_entry or
          self.v25l_brake_entry_counter >= V25L_BRAKE_ENTRY_FRAMES
        ):
      ...
      if self.v25l_brake_active:
        # BukaPilot staging's high-speed reduction is retained, but unlike its
        # gas-car controller V2.5L does not stack a negative desired-speed
        # command on top of hydraulic braking.
        speed_scale = interp(
          CS.out.vEgo,
      ...
      # -----------------------------
      # V2.5L mild regen and positive acceleration
      # -----------------------------
      # Regen is intentionally tiny and lead-only. No-lead negative planner
      # requests produce a neutral desired-speed command, not braking.
      regen_allowed = (
        longitudinal_allowed and
        trusted_lead and
        ...
        frame > self.block_brake_until_frame and
        frame >= self.v25l_brake_reentry_frame and
        brake_request >= V25L_REGEN_ENTRY and
        brake_request < V25L_BRAKE_ENTRY
      )

      if regen_allowed:
        target_regen_offset = -min(
          V25L_REGEN_MAX_OFFSET,
          brake_request * t_lookup,
        )
        self.v25l_regen_offset = max(
          target_regen_offset,
          self.v25l_regen_offset - V25L_REGEN_STEP,
        )
      else:
        # Lead loss, CANCEL, pedals, hydraulic entry, and positive planner
        # demand all remove regenerative braking immediately.
        self.v25l_regen_offset = 0.0

      regen_req = self.v25l_regen_offset < -0.01
      propulsion_blocked = (
        hydraulic_req or
        regen_req or
      ...
      if (
        not longitudinal_allowed or
        propulsion_blocked or
        apply_accel < V25L_ACCEL_ENTRY
      ):
        self.v25l_accel_counter = 0
        self.v25l_accel_cmd = 0.0
      else:
        self.v25l_accel_counter = min(
          self.v25l_accel_counter + 1,
          V25L_ACCEL_ENTRY_FRAMES,
        )
        accel_cap = (
          V25L_ACCEL_CAP_LEAD
          if lead
          else V25L_ACCEL_CAP_NO_LEAD
        )
        target_accel = (
          min(apply_accel, accel_cap)
          if self.v25l_accel_counter >= V25L_ACCEL_ENTRY_FRAMES
          else 0.0
        )
        self.v25l_accel_cmd = v25l_rate_limit_accel(
          target_accel,
          self.v25l_accel_cmd,
        )

      if hydraulic_req:
        # Do not stack regenerative desired-speed braking with 0x271.
        des_speed = CS.out.vEgo
      elif regen_req:
        des_speed = max(0.0, CS.out.vEgo + self.v25l_regen_offset)
      elif self.v25l_accel_cmd > 0.0:
        speed_offset_cap = (
          V25L_SPEED_OFFSET_CAP_LEAD
          if lead
          else V25L_SPEED_OFFSET_CAP_NO_LEAD
        )
        des_speed = CS.out.vEgo + min(
          self.v25l_accel_cmd * t_lookup,
          speed_offset_cap,
        )
      else:
        des_speed = CS.out.vEgo
      ...
      brake_amt_for_hud = (
        self.v25l_apply_brake
        if hydraulic_req
        else (V25L_BRAKE_MIN if regen_req else 0.0)
      )
