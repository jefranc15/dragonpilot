...
def create_can_steer_command(packer, steer, steer_req, raw_cnt):
  """Creates a CAN message for the Dnga LKA Steer Command."""
...
def create_steer_command(packer, command, direction, enable, idx):
  ...
  crc = dnga_checksum(514, dat[:-1])
  values["CHECKSUM_STEERING"] = crc

  return packer.make_can_msg("TORQUE_COMMAND", 0, values)

def dnga_create_gas_command(packer, gas_amount, enable, idx):
  ...
  checksum = dnga_checksum(512, dat[:-1])
...
def dnga_create_accel_command(packer, set_speed, acc_rdy, enabled, is_lead, des_speed, brake_amt, brake_pump, set_distance):
