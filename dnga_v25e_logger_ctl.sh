...
BASE=/data/openpilot
LOGGER="$BASE/dnga_v25e_combined_logger.py"
PIDFILE="$BASE/dnga_v25e_logger.pid"
STATUSFILE="$BASE/dnga_v25e_logger_status.txt"
...
start_logger() {
  ...
  if kill -0 "$PID" 2>/dev/null; then
    echo "v2.5e combined logger started. PID: $PID"
...
stop_logger() {
  ...
  echo
  ls -lht "$BASE"/dnga_v25e_combined_*.csv.gz 2>/dev/null | head -n 3
}
...
case "${1:-status}" in
  ...
  logs) ls -lht "$BASE"/dnga_v25e_combined_*.csv.gz 2>/dev/null | head -n 10 ;;
  *)
    echo "Usage: sh dnga_v25e_logger_ctl.sh {start|stop|restart|status|tail|logs}"
    exit 1
    ;;
esac
